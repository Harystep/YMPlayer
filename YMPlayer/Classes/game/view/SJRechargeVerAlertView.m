...
- (void)configData {
    ...
    [requestModel requestFinish:^(__kindof PPResponseBaseModel * _Nullable responseModel, PPError * _Nullable error) {
        ...
        [data.normal enumerateObjectsUsingBlock:^(PPChargeUnitModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            PPChargetCoinData * itemData = [[PPChargetCoinData alloc] initWithCoinData:obj];
            itemData.payType = [self convertPayTypeWithData:data.paySupport];
...
//            [self hideLoading];
//            [self makeToast:@"支付成功"];
//            [self checkApplePay:data andAppleReceipt:receipt];
//        });
//    } withFaileBlock:^(NSString * _Nonnull errMessage) {
//        dispatch_async(dispatch_get_main_queue(), ^{
//            [self hideLoading];
