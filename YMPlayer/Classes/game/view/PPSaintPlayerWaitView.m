...
- (UIButton * )thePlayer1Button{
  if (!_thePlayer1Button) {
    ...
    [theView setImage:[PPImageUtil imageNamed:@"paly_rule_1"] forState:UIControlStateNormal];
    theView.tag = 1;
...
- (UIButton * )thePlayer2Button{
  if (!_thePlayer2Button) {
    ...
    [self addSubview:theView];
    [theView setImage:[PPImageUtil imageNamed:@"paly_rule_2"] forState:UIControlStateNormal];
    theView.tag = 2;
    [theView addTarget:self action:@selector(onTapPerchSaintPlayerPress:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIButton * )thePlayer3Button{
  if (!_thePlayer3Button) {
    ...
    [self addSubview:theView];
    theView.tag = 3;
    [theView setImage:[PPImageUtil imageNamed:@"paly_rule_3"] forState:UIControlStateNormal];
    [theView addTarget:self action:@selector(onTapPerchSaintPlayerPress:) forControlEvents:UIControlEventTouchUpInside];
...
- (UIButton * )thePlayer4Button{
  if (!_thePlayer4Button) {
    ...
    [self addSubview:theView];
    theView.tag = 4;
    [theView setImage:[PPImageUtil imageNamed:@"paly_rule_4"] forState:UIControlStateNormal];
    [theView addTarget:self action:@selector(onTapPerchSaintPlayerPress:) forControlEvents:UIControlEventTouchUpInside];
