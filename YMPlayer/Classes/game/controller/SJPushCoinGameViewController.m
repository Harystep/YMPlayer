...
#pragma mark - 音量
- (void)onTapVoicePress {
    self.theVoiceButton.selected = !self.theVoiceButton.selected;
    if(self.theVoiceButton.selected) {
        MPMusicPlayerController* musicController = [MPMusicPlayerController applicationMusicPlayer];
        musicController.volume = 0.0;
    } else {
        MPMusicPlayerController* musicController = [MPMusicPlayerController applicationMusicPlayer];
        musicController.volume = 0.25;
    }
...
- (UIButton * )theCloseButton {
    if (!_theCloseButton) {
        ...
        [theView setImage:[PPImageUtil getImage:@"push_coin_exit"] forState:UIControlStateNormal];
...
- (UIButton * )theWarrantyButton {
    if (!_theWarrantyButton) {
        ...
        [self.view addSubview:theView];
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.theVoiceButton.mas_bottom).offset(12);
            make.leading.equalTo(self.view.mas_leading);
        }];
        [theView setImage:[PPImageUtil imageNamed:@"push_coin_repair"] forState:UIControlStateNormal];
...
- (UIButton * )theVoiceButton {
    if (!_theVoiceButton) {
        UIButton * theView = [[UIButton alloc] init];
        [self.view addSubview:theView];
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(self.theGameRuleButton.mas_bottom).offset(12);
            make.leading.equalTo(self.view.mas_leading);
        }];
        [theView setImage:[PPImageUtil imageNamed:@"push_coin_voice"] forState:UIControlStateNormal];
        [theView setImage:[PPImageUtil imageNamed:@"push_coin_voice_no"] forState:UIControlStateSelected];
        [theView addTarget:self action:@selector(onTapVoicePress) forControlEvents:UIControlEventTouchUpInside];
        _theVoiceButton = theView;
    }
    return _theVoiceButton;
}
- (UIButton * )theGameRuleButton{
    if (!_theGameRuleButton) {
        ...
        [theView setImage:[PPImageUtil imageNamed:@"push_coin_rule"] forState:UIControlStateNormal];
...
- (UIButton * )theRechangeButton{
    if (!_theRechangeButton) {
        UIButton * theView = [[UIButton alloc] init];
        [self.view addSubview:theView];
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.view);
            make.bottom.mas_equalTo(self.theRainHangingButton.mas_top).inset(12);
        }];
        [theView setImage:[PPImageUtil imageNamed:@"push_coin_rechange"] forState:UIControlStateNormal];
        [theView addTarget:self action:@selector(onRechangePress) forControlEvents:UIControlEventTouchUpInside];
        _theRechangeButton = theView;
    }
    return _theRechangeButton;
}
- (UIButton * )theRechargeButton{
    if (!_theRechargeButton) {
        ...
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.view);
            make.bottom.mas_equalTo(self.theRechangeButton.mas_top).inset(12);
        }];
        [theView setImage:[PPImageUtil imageNamed:@"push_coin_recharge"] forState:UIControlStateNormal];
...
- (UIButton * )theRainHangingButton{
    if (!_theRainHangingButton) {
        ...
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.trailing.equalTo(self.view);
            make.bottom.mas_equalTo(self.view.mas_bottom).inset(175);
        }];
        theView.hidden = YES;
        [theView setImage:[PPImageUtil imageNamed:@"push_coin_wrapper"] forState:UIControlStateNormal];
...
- (PPChatMessageView * )theChatMessageView{
    if (!_theChatMessageView) {
        ...
        _theChatMessageView.hidden = YES;
...
- (SJUserMoneyDataView * )theMyMoneyView{
    if (!_theMyMoneyView) {
        ...
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.trailing.equalTo(self.view.mas_trailing).inset(12);
