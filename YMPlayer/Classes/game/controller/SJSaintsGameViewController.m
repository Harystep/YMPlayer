...
- (PPSaintPostionPlayingView * )theSaintPostionPlayingView{
    if (!_theSaintPostionPlayingView) {
        ...
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.leading.equalTo(self.theCloseButton.mas_trailing).offset(DSize(30));
        }];
        @weakify_self;
...
- (UIButton * )theGameRuleButton{
    if (!_theGameRuleButton) {
        ...
        [self.view addSubview:theView];
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.leading.equalTo(self.theGameExchangeButton.mas_trailing).offset(10);
            make.centerY.equalTo(self.theCloseButton.mas_centerY);
            make.size.mas_equalTo(CGSizeMake(DSize(52), DSize(52)));
        }];
        [theView setImage:[PPImageUtil imageNamed:@"player_rule_icon"] forState:UIControlStateNormal];
        ...
        [self createOpView:theView title:@"规则"];
    }
...
- (UIButton *)theGameRechargeButton{
    if (!_theGameRechargeButton) {
        ...
        [self.view addSubview:theView];
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.leading.equalTo(self.theSaintPostionPlayingView.mas_trailing).offset(30);
            make.centerY.equalTo(self.theCloseButton.mas_centerY);
            make.size.mas_equalTo(CGSizeMake(DSize(52), DSize(52)));
        }];
        [theView setImage:[PPImageUtil imageNamed:@"player_recharge_icon"] forState:UIControlStateNormal];
        ...
        [self createOpView:theView title:@"充值"];
    }
...
- (UIButton *)theGameExchangeButton{
    if (!_theGameExchangeButton) {
        ...
        [self.view addSubview:theView];
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.leading.equalTo(self.theGameRechargeButton.mas_trailing).offset(10);
            make.centerY.equalTo(self.theCloseButton.mas_centerY);
            make.size.mas_equalTo(CGSizeMake(DSize(52), DSize(52)));
        }];
        [theView setImage:[PPImageUtil imageNamed:@"player_rechange_icon"] forState:UIControlStateNormal];
        ...
        [self createOpView:theView title:@"兑换"];
    }
...
- (UIView *)coinContentBgView{
    if (!_coinContentBgView) {
        ...
        [_coinContentBgView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.trailing.mas_equalTo(self.view.mas_trailing).inset(DSize(30));
            make.top.mas_equalTo(self.view.mas_top).offset(DSize(30));
...
- (UIButton * )theVoiceButton {
    if (!_theVoiceButton) {
        ...
        [self.view addSubview:theView];
        [theView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.leading.equalTo(self.theGameRuleButton.mas_trailing).offset(10);
            make.centerY.equalTo(self.theCloseButton.mas_centerY);
            make.width.height.mas_equalTo(DSize(52));
        }];
        [theView setImage:[PPImageUtil imageNamed:@"player_voice_icon"] forState:UIControlStateNormal];
        [theView setImage:[PPImageUtil imageNamed:@"player_voice_icon_no"] forState:UIControlStateSelected];
        ...
        [self createOpView:theView title:@"声音"];
    }
    return _theVoiceButton;
}

- (void)createOpView:(UIView *)theView title:(NSString *)title {
    UILabel *lb = [[UILabel alloc] init];
    [theView addSubview:lb];
    [lb mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.mas_equalTo(theView);
        make.top.mas_equalTo(theView.mas_bottom).inset(-6);
        make.width.mas_equalTo(theView.mas_width);
        make.height.mas_equalTo(13);
    }];
    lb.font = [UIFont systemFontOfSize:10];
    lb.textAlignment = NSTextAlignmentCenter;
    lb.backgroundColor = RGBACOLOR(0, 0, 0, 0.59);
    lb.layer.cornerRadius = 3;
    lb.layer.masksToBounds = YES;
    lb.textColor = UIColor.whiteColor;
    lb.text = ZCLocal(title);
}
